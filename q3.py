...
for i in range(nrows):
    ...
    for p in potential_pairs:
        ...
        if latlong_dist(loc1, loc2) <= 0.06:
            m.addConstr(small[p[0]] + med[p[0]] + large[p[0]] + small[p[1]] + med[p[1]] + large[p[1]] <= 1,
                        name=f'too_close_{potential_ids[p[0]]}_{potential_ids[p[1]]}_{int(zip1)}')
    ...
    for p in potential_existing_pairs:
        ...
        if latlong_dist(loc1, loc2) <= 0.06:
            m.addConstr(small[p[0]] + med[p[0]] + large[p[0]] == 0,
                        name=f'too_close_{potential_ids[p[0]]}_{facility_ids[p[1]]}_{int(zip1)}')
    ...
    if len(current['facility_id']) > 0:
        ...
        for k in range(len(expans05)):
            ...
            # Binary variables to do piecewise cost, w indicates what bracket the expansion rate is in
            w1 = m.addVar(vtype=GRB.BINARY, name=f'{zip1}_{facility_ids[k]}_exp_<0.10')
            w2 = m.addVar(vtype=GRB.BINARY, name=f'{zip1}_{facility_ids[k]}_exp_0.10-0.15')
            w3 = m.addVar(vtype=GRB.BINARY, name=f'{zip1}_{facility_ids[k]}_exp_>0.15')

            # Constraints to implement binary indicators for each range of expansion
            m.addConstr(w1 + w2 + w3 == 1, name=f'{zip1}_{facility_ids[k]}_mutex')
            m.addConstr(exp_rate <= 0.1 + M * (1 - w1), name=f'{zip1}_{facility_ids[k]}_aux1')
            m.addConstr(exp_rate >= 0.1 * w2, name=f'{zip1}_{facility_ids[k]}_aux2')
            m.addConstr(exp_rate <= 0.15 + M * (1 - w2), name=f'{zip1}_{facility_ids[k]}_aux3')
            m.addConstr(exp_rate >= 0.15 * w3, name=f'{zip1}_{facility_ids[k]}_aux4')
    ...
    coverage_rate_li.append(new_totalcapacity_byzip_exp/master_df['0-12 pop'].iloc[i])
    total_slots += new_totalcapacity_byzip_exp

for i in range(len(coverage_rate_li)):
    for j in range(len(coverage_rate_li)):
        m.addConstr(coverage_rate_li[i] - coverage_rate_li[j] <= 0.1, name='Faireness')
m.addConstr(master_cost_objective <= 1000000000, name='Budget')

m.setObjective(total_slots/master_df['0-12 pop'].sum(), GRB.MAXIMIZE)
m.update()
m.optimize()
...
for i in range(nrows):
    ...
    row = {
        ...
        'new_total_05': 50 * small_buildings + 100 * med_buildings + 200 * large_buildings + zerofive_added + sum(
            current['0-5 capacity']),
        'new_total_012': sum(current[
                                 'total_capacity']) + zerofive_added + fivetwelve_added + 100 * small_buildings + 200 * med_buildings + 400 * large_buildings,
